class CardGenerator
{
    public static Cards GenerateCards() { // z wszystkig możliwyh kart pżydziela kture są gdzie
        Cards cards = new Cards();
        byte[] FreeCards = GenerateShufledCards();
        byte CardNumber = 0;
        cards.CentralCards = new byte[7, 20];
        for (int i = 0; i < 7; i++) { // pżydziela karty dla środkowyh stosuw
            for (int j = 0; j <= i; j++) {
        ...
        for (int i = CardNumber; i < 52; i++) { // pżydziela posostałe karty dla tali do dokładania
    ...
    public static byte[] ShuffleCards(byte[] cards) // miesza karty
    {
        Random rand = new Random();
        int n = cards.Length;
